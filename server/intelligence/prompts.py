...
def get_tailored_cv_messages(cv_data, job_title, job_description, language="de", cv_notes: str = ""):
    ...
    system = (
        ...
        "job posting. Your tasks:\n"
        "1. Tailor and reorder existing content to best match the job.\n"
        "2. Write a compelling 'summary' (2-3 sentences) that uses the candidate's name, role, "
        "location, skills, and spoken_languages exactly as provided in the input JSON.\n"
        "3. Complete any sparse or missing descriptions using ONLY facts already present in the input.\n"
        "You MUST NOT invent employers, dates, or skills that are not in the input JSON. "
        "Return ONLY a valid JSON object with the same keys as the input (name, role, location, "
        "skills, spoken_languages, experience, projects, education) plus 'summary'. " + lang_note
    )
    user = (
