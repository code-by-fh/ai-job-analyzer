...
export default async function Page({ searchParams }: PageProps) {
  ...
  const initialFilter = validFilters.includes(rawFilter || "")
    ? (rawFilter as any)
    : "all";

  const platformIdParam = params?.platform_id;
  const rawPlatformId = Array.isArray(platformIdParam)
    ? platformIdParam[0]
    : platformIdParam;
  const initialPlatformId = rawPlatformId
    ? parseInt(rawPlatformId, 10)
    : undefined;
  const platformNameParam = params?.platform_name;
  const initialPlatformName = (
    Array.isArray(platformNameParam) ? platformNameParam[0] : platformNameParam
  ) as string | undefined;
