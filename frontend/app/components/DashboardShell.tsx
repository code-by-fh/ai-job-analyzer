...
export default function DashboardShell({
  ...
  const { t } = useLanguage();
  const { errorDetail, clearError, showError, successDetail, clearSuccess } =
    useNotification();
  ...
  React.useEffect(() => {
    ...
    fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/status`)
      .then((r) => r.json())
      .then((data) => {
        if (data.ai_error) showError(data.ai_error);
      })
      .catch(() => {});
  }, [user]);
  ...
  const label = (item: NavItemConfig) =>
    item.labelKey ? t(item.labelKey) : item.labelLiteral || "";
  const hasAdmin = Boolean(user?.is_admin);
  const adminActive =
    hasAdmin && ADMIN_NAV_ITEMS.some((item) => isActive(item));
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            <>
              <div className={`py-3 ${sidebarCollapsed ? "" : "px-2"}`}>
                {sidebarCollapsed ? (
                  ...
                  <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-600 uppercase tracking-widest">
                    Admin
                  </p>
                )}
              </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 h-16 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 flex items-stretch">
        ...
        >
          ...
          <span
            className={`transition-transform duration-200 ${moreOpen ? "rotate-90 scale-110" : ""}`}
          >
      ...
      {/* ── MAIN CONTENT ── */}
      <main
        className={`flex-1 w-full transition-all duration-300 ease-in-out ${sidebarCollapsed ? "md:pl-[68px]" : "md:pl-56"}`}
      >
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-20 md:pt-8 pb-24 md:pb-8 min-h-screen">
          ...
          {successDetail && (
            <div className="mb-6">
              <SuccessBanner detail={successDetail} onDismiss={clearSuccess} />
            </div>
          )}
          {children}
        </div>
...
function SidebarLink({
  ...
  return (
    ...
    >
      ...
      <span
        className={`flex-shrink-0 transition-transform duration-200 group-hover:scale-110 ${active ? "scale-110" : ""}`}
      >
