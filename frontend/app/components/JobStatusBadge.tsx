...
export default function JobStatusBadge({
  ...
  const getStatusConfig = (s: string) => {
    ...
    const base = `border font-bold uppercase tracking-wider transition-all duration-300 backdrop-blur-md shadow-sm flex items-center justify-center gap-2 ${
      isLarge
    ...
    switch (s) {
      case "OPEN":
        return {
          ...
          color:
            "bg-slate-50 border-slate-200 text-slate-500 dark:bg-slate-800/30 dark:border-slate-700/50 dark:text-slate-400",
      ...
      case "ACCEPTED":
        return {
          ...
          color:
            "bg-teal-50 border-teal-100 text-teal-600 dark:bg-teal-500/10 dark:border-teal-500/20 dark:text-teal-400",
      ...
      case "GENERATING":
        return {
          ...
          color:
            "bg-indigo-600 text-white border-transparent animate-pulse shadow-indigo-500/20",
  ...
  if (isReadOnly || status === "GENERATING") {
    return (
      <div
        className={`flex items-center justify-center gap-2 rounded-lg ${config.className} ${config.color}`}
      >
        {config.icon}
  ...
  return (
    <div
      className={`relative ${size === "large" ? "flex-shrink-0 w-full sm:w-auto" : ""}`}
    >
      <button
      ...
      >
        ...
        <ChevronDown
          size={10}
          className={`opacity-40 ml-1 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
