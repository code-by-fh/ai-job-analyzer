...
export default function JobCompanyTab({ job, apiBase }: JobCompanyTabProps) {
  ...
  useEffect(() => {
    if (job.company_domain && !companyData && !companyLoading) {
      ...
      fetchWithAuth(`${apiBase}/companies/${job.company_domain}`)
        ...
        .catch(() =>
          showError(`GET /companies/${job.company_domain} fehlgeschlagen`),
        )
  ...
  useEffect(() => {
    ...
    if (!stored)
      localStorage.setItem(`gen_company_${job.id}`, startTime.toString());
  ...
  const handleUpdate = () => {
    ...
    fetchWithAuth(`${apiBase}/companies/${job.company_domain}/analyze`, {
      ...
      .then((res) => {
        if (!res.ok)
          throw new Error(
            `POST /companies/${job.company_domain}/analyze → HTTP ${res.status}`,
          );
      })
      .catch((e: Error) => {
        setCompanyQueued(false);
      ...
      <div className="group relative flex flex-col items-center justify-center py-16 gap-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl transition-all hover:border-indigo-400 dark:hover:border-indigo-500/50 bg-slate-50/50 dark:bg-slate-900/20">
        ...
        <div className="text-center px-6 max-w-sm space-y-2">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Erhalte exklusive Einblicke in SWOT, Kultur, Gehaltsbenchmarks und
            Marktpositionierung.
          </p>
        </div>
        <button
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toolbar */}
      <div
        className={`flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800/50 transition-opacity duration-300 ${companyQueued ? "opacity-40 pointer-events-none select-none" : ""}`}
      >
      ...
      <div
        className={`transition-opacity duration-300 ${companyQueued ? "opacity-40 pointer-events-none select-none" : ""}`}
      >
        <CompanyProfileView
          data={d}
          domain={d.domain}
