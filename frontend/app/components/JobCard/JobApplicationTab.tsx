...
export default function JobApplicationTab({
  ...
  useEffect(() => {
    ...
    if (!stored)
      localStorage.setItem(`gen_app_${job.id}`, startTime.toString());
  ...
  const regenBanner = isGenerating ? (
    <RegenBanner
      label={regenPhase.label}
      icon={regenPhase.icon}
      elapsed={elapsed}
      ...
      {job.application_draft ? (
        <div className="space-y-4">
            ...
            <div className="bg-purple-50 dark:bg-purple-500/5 border border-purple-200 dark:border-purple-500/20 rounded-xl p-4 space-y-3">
              ...
              <div className="flex gap-2 justify-end">
                ...
                >
                  {isSubmittingRegen ? (
                    <>
                      <Loader2 className="w-3 h-3 animate-spin" /> Wird
                      gestartet…
                    </>
                  ) : (
                    <>
          ...
          {isEditing ? (
            ...
            <div
              className={`bg-white dark:bg-slate-800 p-8 md:p-10 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg font-serif relative transition-opacity duration-300 ${isGenerating ? "opacity-40 pointer-events-none select-none" : ""}`}
            >
        ...
        <div className="group relative flex-1 flex flex-col items-center justify-center py-16 gap-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl transition-all hover:border-indigo-400 dark:hover:border-indigo-500/50 bg-slate-50/50 dark:bg-slate-900/20">
          ...
          <div className="text-center px-6 max-w-sm space-y-2">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              KI-generiertes und auf dich abgestimmtes Anschreiben basierend auf
              deinem Profil.
            </p>
          </div>
          <button
