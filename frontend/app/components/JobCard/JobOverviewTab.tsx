...
export default function JobOverviewTab({
  ...
  const handleTriggerAnalysis = async () => {
    ...
    try {
      await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs/${job.id}/analyze`,
        { method: "POST" },
      );
    } catch {
      ...
      >
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 sm:gap-4">
          ...
          <div className="flex items-center h-10 gap-3 bg-white dark:bg-slate-800 px-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <span
              className={`text-lg font-black ${scoreTextColor} tracking-tighter`}
            >
              {score}%
            </span>
            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest border-l border-slate-200 dark:border-slate-700 pl-3">
              Match
            </span>
          </div>
          ...
          {/* Status*/}
          <span
            className={`inline-flex items-center h-10 gap-2 px-4 rounded-xl border text-[10px] font-black uppercase tracking-wider ${statusMeta.pillCls} shadow-sm`}
          >
        ...
        <div className="w-full sm:w-52 flex-shrink-0 space-y-3">
          ...
          <div className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 p-3">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className={`text-sm font-black ${scoreTextColor}`}>
                {score}%
              </span>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 p-3 space-y-2">
            ...
            {onStatusUpdate ? (
                      ...
                      {ALL_STATUS_KEYS.map((s) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isActive && (
                              <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 ring-4 ring-indigo-500/20" />
                            )}
                          </button>
                        );
                      })}
              ...
              <span
                className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full border ${statusMeta.pillCls}`}
              >
                <DynamicIcon name={statusMeta.icon} className="w-3.5 h-3.5" />{" "}
                {t(statusMeta.labelKey)}
              </span>
...
function CheckItem({
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[11px] font-medium transition-colors ${done ? "text-slate-600 dark:text-slate-300" : "text-slate-400 dark:text-slate-500 group-hover/ci:text-indigo-600 dark:group-hover/ci:text-indigo-400"}`}
      >
        {label}
      </span>
      <svg
        className="w-3 h-3 ml-auto opacity-0 group-hover/ci:opacity-60 text-slate-400 dark:text-slate-500 transition-opacity flex-shrink-0"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
        strokeWidth={2}
