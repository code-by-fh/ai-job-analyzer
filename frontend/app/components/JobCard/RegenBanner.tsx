...
export default function RegenBanner({ label, icon, elapsed, onCancel, phaseCount, phaseIndex }: RegenBannerProps) {
        ...
        <div className="flex items-center justify-between gap-3 px-4 py-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30 rounded-xl">
            <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                    <p className="text-xs font-bold text-indigo-700 dark:text-indigo-300 truncate">{label}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                        {showPhases && (
                            <div className="flex gap-0.5">
                                {Array.from({ length: phaseCount! }).map((_, i) => (
                                    <div key={i} className={`h-1 rounded-full transition-all duration-500 ${
                                        i === phaseIndex ? 'w-4 bg-indigo-500'
                                        : i < phaseIndex! ? 'w-1 bg-indigo-300 dark:bg-indigo-600'
                                        : 'w-1 bg-indigo-100 dark:bg-indigo-500/20'
                                    }`} />
                                ))}
                            </div>
                        )}
                        <span className="text-[10px] text-indigo-400 dark:text-indigo-500 tabular-nums">{formatElapsed(elapsed)}</span>
                    </div>
                </div>
            </div>
            <button
