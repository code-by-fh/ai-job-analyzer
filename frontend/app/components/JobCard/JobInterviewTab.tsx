...
function Section({
  ...
  const bg: Record<string, string> = {
    slate:
      "bg-slate-50/50 dark:bg-slate-800/30 border-slate-200 dark:border-slate-700/50 shadow-sm",
    indigo:
      "bg-indigo-50/50 dark:bg-indigo-500/5 border-indigo-200 dark:border-indigo-500/20 shadow-sm",
    emerald:
      "bg-emerald-50/50 dark:bg-emerald-500/5 border-emerald-200 dark:border-emerald-500/20 shadow-sm",
    ...
    purple:
      "bg-purple-50/50 dark:bg-purple-500/5 border-purple-200 dark:border-purple-500/20 shadow-sm",
  };
  ...
  return (
    <div
      className={`rounded-2xl border p-5 ${bg[color]} transition-all hover:shadow-md`}
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3
          className={`text-base font-bold uppercase tracking-widest ${tc[color]}`}
        >
          {title}
        </h3>
      </div>
...
function GapSeverityBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`text-xs uppercase font-black px-3 py-1 rounded-lg shadow-sm ${map[level] ?? map.Medium}`}
    >
      {level}
    </span>
  );
...
function DeepDiveItem({
  ...
  const handleResearch = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${apiBase}/companies/${domain}/deep-dive`,
        {
          method: "POST",
  ...
  return (
    <div
      className={`rounded-xl border overflow-hidden transition-all duration-200 ${open ? "border-indigo-400 dark:border-indigo-500/50 shadow-md" : "border-indigo-200/70 dark:border-indigo-500/20 hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:shadow-sm cursor-pointer"}`}
    >
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <ChevronDown
          className={`w-4 h-4 text-indigo-400 flex-shrink-0 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
        <div className="px-5 pb-5 pt-4 space-y-4 bg-white/80 dark:bg-slate-900/60 border-t border-indigo-100/60 dark:border-indigo-500/10 animate-in slide-in-from-top-2 duration-200">
          {btn.focus && (
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mb-1.5">
                  Focus
                </p>
          ...
          {btn.how_to_proceed && (
            <div className="flex gap-3">
              ...
              <div>
                <p className="text-[10px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest mb-1.5">
...
export default function JobInterviewTab({
  job,
  apiBase,
}: JobInterviewTabProps) {
  ...
  useEffect(() => {
    ...
    if (!stored)
      localStorage.setItem(`gen_interview_${job.id}`, startTime.toString());
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (updatedJob.interview_prep_material) {
          try {
            setInterviewPrep(JSON.parse(updatedJob.interview_prep_material));
          } catch {
            setInterviewPrep({ raw: updatedJob.interview_prep_material });
          }
          setInterviewQueued(false);
          localStorage.removeItem(`gen_interview_${job.id}`);
        }
  ...
  const regenBanner =
    interviewQueued && interviewPrep ? (
      <RegenBanner
        label="Interview-Analyse läuft…"
      ...
      <div className="group relative flex flex-col items-center justify-center py-16 gap-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl transition-all hover:border-indigo-400 dark:hover:border-indigo-500/50 bg-slate-50/50 dark:bg-slate-900/20">
        ...
        <div className="text-center px-6 max-w-sm space-y-2">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Erhalte eine personalisierte Vorbereitung basierend auf deinem
            Profil und den Job-Anforderungen.
          </p>
        </div>
        <button
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Toolbar */}
      <div
        className={`flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800/50 transition-opacity duration-300 ${interviewQueued ? "opacity-40 pointer-events-none select-none" : ""}`}
      >
      ...
      <div
        className={`space-y-5 transition-opacity duration-300 ${interviewQueued ? "opacity-40 pointer-events-none select-none" : ""}`}
      >
        {/* Executive Summary */}
          ...
          >
            <div className="space-y-3">
              {p.structured_prep.gap_analysis.map((gap: any, i: number) => (
                <div
                  key={i}
                  className="p-4 bg-white/60 dark:bg-slate-900/40 rounded-xl border border-amber-200/50 dark:border-amber-500/15"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    {gap.gap_severity && (
                      <GapSeverityBadge level={gap.gap_severity} />
                    )}
                  </div>
                  {gap.cv_status && (
        ...
        {p.social_intelligence_research && (
          ...
          >
            <div className="space-y-4">
              {p.social_intelligence_research.potential_contacts?.length >
                0 && (
                <div>
                  ...
                  <ul className="space-y-1">
                    {p.social_intelligence_research.potential_contacts.map(
                      (c: any, i: number) => (
                        <li
                          key={i}
              ...
              {p.social_intelligence_research.insights_from_contacts?.length >
                0 && (
                <div className="p-3 bg-sky-100/50 dark:bg-sky-500/10 rounded-xl border border-sky-200/60 dark:border-sky-500/20">
                  <p className="text-[10px] font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest mb-2">
                    Insights aus Kontakten
                  </p>
                  <ul className="space-y-1">
                    {p.social_intelligence_research.insights_from_contacts.map(
                      (c: any, i: number) => (
              ...
              {p.social_intelligence_research.research_sources?.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-1">
                    {p.social_intelligence_research.research_sources.map(
                      (src: any, i: number) => (
            ...
            <div className="space-y-3">
              {p.confidence_assessment.overall_confidence && (
                <div className="flex items-center gap-3">
                  ...
                  <GapSeverityBadge
                    level={p.confidence_assessment.overall_confidence}
                  />
                </div>
              )}
              {p.confidence_assessment.uncertainties?.length > 0 && (
                <ul className="space-y-1 mt-2">
                  {p.confidence_assessment.uncertainties.map(
                    (u: any, i: number) => (
                      <li
                        key={i}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-black uppercase tracking-widest text-indigo-700 dark:text-indigo-300">
                    Taktische Deep-Dives
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                    Spezifische Taktiken & psychologische Manöver für dein
                    Interview
                  </p>
                </div>
