...
export default function BoardJobCard({ job, onClick }: BoardJobCardProps) {
  ...
  const matchScoreStr = job.match_score
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start gap-2 mb-2">
        <h4
          className="font-bold text-sm text-slate-800 dark:text-slate-100 line-clamp-2 leading-tight"
          title={job.title}
        >
          {job.title}
        </h4>
      ...
      <p
        className="text-xs font-semibold text-slate-600 dark:text-slate-400 truncate mb-3"
        title={job.company}
      >
        ...
        <div className="flex items-center gap-1.5 opacity-70 group-hover:opacity-100 transition-opacity">
          ...
          {statusMeta && (
            <DynamicIcon
              ...
              className={`w-3.5 h-3.5 ${statusMeta.pillCls.split(" ")[1]}`}
            />
          )}
        </div>
      </div>
