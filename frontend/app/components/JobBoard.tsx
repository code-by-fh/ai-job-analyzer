          ...
          const columnJobs = jobs.filter((j) => {
            ...
            if (status === "OPEN") {
              return (
                ...
                !KNOWN_STATUSES.has(jobStatus as any)
              );
            }
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`w-8 h-8 rounded-xl flex items-center justify-center border shadow-sm ${meta.pillCls}`}
                  >
              ...
              <div className="flex-1 p-3 overflow-y-auto space-y-3 min-h-[150px] flex flex-col">
                {isArchive ? (
                  ...
                  >
                    <div
                      className={`text-4xl font-extrabold ${isDragOver ? "text-indigo-500 scale-125 animate-pulse" : "text-slate-400 dark:text-slate-600"}`}
                    >
                      {displayCount}
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                        {isDragOver
                          ? t("dropToArchive" as any) || "Release to Archive"
                          : t("dragToArchive" as any) || "Drag jobs here"}
                      </p>
                    </div>
                  </div>
                ) : (
