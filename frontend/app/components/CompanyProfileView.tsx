...
function Section({
  ...
  return (
    <div
      className={`rounded-2xl border p-5 shadow-sm ${bg[color]} transition-all hover:shadow-md`}
    >
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3
          className={`text-sm font-bold uppercase tracking-widest ${tc[color]}`}
        >
          {title}
        </h3>
      </div>
...
function ConfidenceBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${map[level] ?? map.Moderate}`}
    >
      {level}
    </span>
  );
...
function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${map[level] ?? map.Medium}`}
    >
      {level}
    </span>
  );
...
function AnalysisArea({ area }: { area: any }) {
  ...
  return (
    <div
      className={`rounded-xl border overflow-hidden transition-all duration-200 ${open ? "border-indigo-400 dark:border-indigo-500/50" : "border-slate-200 dark:border-slate-700/50 hover:border-indigo-300 dark:hover:border-indigo-500/40"}`}
    >
      <button
        onClick={() => setOpen((v) => !v)}
        ...
        <div className="px-5 pb-5 pt-4 space-y-3 bg-white/80 dark:bg-slate-900/60 border-t border-slate-100/60 dark:border-slate-700/20 animate-in slide-in-from-top-2 duration-200">
          ...
          {area.evidence_basis && (
            <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-200/60 dark:border-slate-700/30">
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">
                Quellenbasis
              </p>
...
function DeepDiveButton({
  ...
  const API_BASE =
    ...
    (typeof window !== "undefined"
      ? (window as any).__ENV__?.APP_API_URL
      : "") ||
    "";
  ...
  const handleResearch = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE}/companies/${domain}/deep-dive`,
        {
          method: "POST",
  ...
  return (
    <div
      className={`rounded-xl border overflow-hidden transition-all duration-200 ${open ? "border-indigo-400 dark:border-indigo-500/50 shadow-md" : "border-indigo-200/70 dark:border-indigo-500/20 hover:border-indigo-300 dark:hover:border-indigo-500/40 hover:shadow-sm"}`}
    >
      <button
        onClick={() => setOpen((v) => !v)}
          ...
          <div className="space-y-4">
            ...
            {d.key_insights.interpretations?.length > 0 && (
              <div className="p-3 bg-amber-100/40 dark:bg-amber-500/10 rounded-xl border border-amber-200/50 dark:border-amber-500/15">
                ...
                <ul className="space-y-1">
                  {d.key_insights.interpretations.map(
                    (item: any, i: number) => (
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {d.key_insights.uncertainties.map((u: any, i: number) => (
                    <li
                      key={i}
                      className="flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400 italic"
                    >
        ...
        >
          <div className="space-y-3">
            {d.risks.map((risk: any, i: number) => (
              <div
                key={i}
                className="p-4 bg-white/60 dark:bg-slate-900/40 rounded-xl border border-rose-200/50 dark:border-rose-500/15"
              >
        ...
        >
          <div className="space-y-4">
            ...
            {d.market_comparison.comparison_points?.length > 0 && (
              <div className="space-y-2">
                {d.market_comparison.comparison_points.map(
                  (cp: any, i: number) => (
                    <div
                      key={i}
                      className="p-3 bg-sky-50/50 dark:bg-sky-500/5 rounded-xl border border-sky-200/50 dark:border-sky-500/15"
                    >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  Spezifische Aspekte des Unternehmens detailliert recherchieren
                </p>
              </div>
          ...
          if (typeof res === "string" && res.startsWith("http")) {
            ...
            urls = res
              .map((r: any) =>
                typeof r === "string"
                  ? r
                  : r?.url || r?.speaking_url || JSON.stringify(r),
              )
              .filter(Boolean);
          } else if (typeof res === "object") {
            urls = Object.values(res).filter(
