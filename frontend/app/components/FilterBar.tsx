...
import {
    Search, FileText, X, Building2, Tag, Clock,
    ChevronDown, LayoutGrid, SlidersHorizontal,
} from 'lucide-react';
...
export default function FilterBar({
    ...
    const hasActiveFilters = searchText || domainFilter || statusFilter || hasApplication || filterType !== 'all' || platformFilter !== undefined;
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <div className="flex bg-slate-100/80 dark:bg-slate-800/50 backdrop-blur-md p-1 rounded-2xl border border-slate-200/60 dark:border-slate-700/40 shadow-sm shrink-0">
                    {[
                        ...
                        <button
                            ...
                            className={`flex-1 sm:flex-none px-5 py-2 text-sm font-bold rounded-xl transition-all duration-300 whitespace-nowrap cursor-pointer ${filterType === tab.id
                ...
                <div className="flex bg-slate-100/80 dark:bg-slate-800/50 p-1 rounded-2xl border border-slate-200/60 dark:border-slate-700/40 shadow-sm shrink-0">
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-xl transition-all duration-300 cursor-pointer ${sortBy === 'score'
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-xl transition-all duration-300 cursor-pointer ${sortBy === 'date'
                            ? 'bg-white dark:bg-slate-700 shadow-md text-indigo-600 dark:text-indigo-400 scale-[1.02]'
                            : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'
                            }`}
                        title={t('newest')}
            ...
            <div className="flex flex-wrap gap-3 items-center">
                ...
                <div className="relative flex-1 min-w-[130px]" ref={statusDropdownRef}>
                    ...
                    >
                        {selectedStatusMeta ? (
                            <DynamicIcon name={selectedStatusMeta.icon} className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-500" />
                        ) : (
                            <Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        )}
                        <span className="flex-1 text-left truncate">
                            {selectedStatusMeta ? t(selectedStatusMeta.labelKey) : t('allStatuses')}
                        </span>
                        <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 shrink-0 ${isStatusOpen ? 'rotate-180' : ''}`} />
                    </button>
                    ...
                    {isStatusOpen && (
                        <div className="absolute left-0 right-0 mt-1.5 p-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl z-[150]">
                            <button
                                ...
                                className={`flex items-center gap-2 w-full px-3 py-2 rounded-xl text-[13px] font-bold transition-all ${!statusFilter
                                ...
                                {JOB_STATUSES.map(s => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex items-center gap-2 w-full px-2.5 py-2 rounded-xl text-[12px] font-bold transition-all ${isActive
                                                ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400'
                                                : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                                        >
                                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center border transition-colors ${isActive
                                            ...
                                            {statusCounts[s] != null && (
                                                <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded-md ${isActive ? 'bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
                                                    {statusCounts[s]}
                                                </span>
                                            )}
                                        </button>
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 text-sm font-bold rounded-xl border transition-all duration-200 cursor-pointer whitespace-nowrap shadow-sm active:scale-95 shrink-0 ${hasApplication
