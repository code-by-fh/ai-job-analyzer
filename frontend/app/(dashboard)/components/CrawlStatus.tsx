...
export function CrawlSteps({
  ...
  const isScraping =
    job.scraping_completed > 0 &&
    job.scraping_completed < job.total &&
    !isFailed;
  const isScrapingDone = job.scraping_completed >= job.total && job.total > 0;
  const isExtracting =
    (job.extracting_count ?? 0) > 0 && !isScrapingDone && !isFailed;
  const isExtractionDone = isScrapingDone;
  const isAnalyzing =
    (job.analyzing_jobs && job.analyzing_jobs.length > 0) ||
    (job.analysis_completed > 0 && !job.show_success && !isFailed);
  ...
  return (
    <div className={`${compact ? "space-y-2.5" : "space-y-4"} relative`}>
      <div
        className={`absolute ${compact ? "left-[7px]" : "left-[11px]"} top-2 bottom-2 w-0.5 bg-slate-100 dark:bg-slate-800 -z-10`}
      />

      {/* Step 1: Suche */}
      <div className={`flex items-center ${gap}`}>
        <div
          className={`relative z-10 flex items-center justify-center ${ic} rounded-full border-2 transition-colors duration-300 ${
            isSearching
        ...
        <span
          className={`${tc} ${isSearching ? "font-medium text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-500"}`}
        >
          {t("searchingForJobs")}
        </span>
      </div>
      ...
      <div className={`flex items-center ${gap}`}>
        ...
        <div className="flex items-center gap-2">
          <span
            className={`${tc} ${isFound ? "font-medium text-slate-900 dark:text-white" : "text-slate-400 dark:text-slate-600"}`}
          >
            {isFound
              ? `${job.total_found ?? job.total} ${t("jobsFound")}`
              : t("jobsFound")}
          </span>
          {isFound && !compact && (
      ...
      <div className={`flex items-center ${gap}`}>
        ...
        <span
          className={`${tc} ${isExtracting ? "font-medium text-violet-600 dark:text-violet-400" : isExtractionDone ? "text-slate-500 dark:text-slate-500" : "text-slate-400 dark:text-slate-600"}`}
        >
          {isExtracting
            ? t("aiExtractionCount", {
                count: job.extracting_count ?? 0,
                total: job.total,
              })
            : t("aiExtraction")}
        </span>
      ...
      <div className={`flex items-center ${gap}`}>
        ...
        <span
          className={`${tc} ${isScraping ? "font-medium text-indigo-600 dark:text-indigo-400" : isScrapingDone ? "text-slate-500 dark:text-slate-500" : "text-slate-400 dark:text-slate-600"}`}
        >
          {isScraping || isScrapingDone
            ? t("processingJobDetails", {
                count: job.scraping_completed,
                total: job.total,
              })
            : t("loadJobDetails")}
        </span>
      ...
      <div className={`flex items-center ${gap}`}>
        ...
        <div className="flex flex-col flex-1">
          <span
            className={`${tc} ${isAnalyzing && !isAnalysisDone ? "font-medium text-amber-600 dark:text-amber-400" : isAnalysisDone ? "text-slate-500 dark:text-slate-500" : "text-slate-400 dark:text-slate-600"}`}
          >
            {isAnalyzing && !isAnalysisDone
              ? t("analyzingCount", {
                  count: (job?.jobs_saved ?? 0) + (job?.jobs_skipped ?? 0),
                  total: job.total,
                })
              : isAnalysisDone
      ...
      {job.show_success && (
        <div
          className={`${compact ? "mt-2 p-2" : "mt-4 p-4"} bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-lg`}
        >
          <div className="flex items-center gap-2">
            <CheckCircle
              className={`${compact ? "w-3.5 h-3.5" : "w-5 h-5"} text-emerald-600 dark:text-emerald-400`}
            />
            <div>
              <p
                className={`${tc} font-semibold text-emerald-700 dark:text-emerald-300`}
              >
                {t("allJobsAnalyzed")}
              </p>
              {!compact && (
      ...
      {isFailed && (
        <div
          className={`${compact ? "mt-2 p-2.5" : "mt-4 p-4"} bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-lg`}
        >
          <div className={`flex flex-col ${compact ? "gap-1.5" : "gap-3"}`}>
            <div className="flex items-start gap-2">
              <svg
                className={`${compact ? "w-3.5 h-3.5" : "w-5 h-5"} text-rose-600 dark:text-rose-400 shrink-0 mt-0.5`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
              ...
              <div>
                ...
                <p
                  className={`${compact ? "text-xs" : "text-xs mt-1"} text-rose-600 dark:text-rose-400 break-words`}
                >
