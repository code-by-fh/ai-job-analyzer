    ...
    return (
        ...
        >
            ...
            <div className="px-3 py-2.5 flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            {isEditingName ? (
                                <div className="flex items-center gap-1">
                                    ...
                                    />
                                    <button onClick={handleNameSubmit} className="p-0.5 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded cursor-pointer">
                                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                                    </button>
                                    <button onClick={() => { setEditedName(platform.name); setIsEditingName(false); }} className="p-0.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded cursor-pointer">
                                ...
                                <div className="flex items-center gap-1.5 flex-wrap group/name">
                                    ...
                                    <div className="flex items-center gap-0.5 opacity-0 group-hover/name:opacity-100 transition-opacity">
                                        <button onClick={() => !isBusy && setIsEditingName(true)} disabled={isBusy} className="p-0.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded text-slate-400 hover:text-indigo-500 transition-colors cursor-pointer disabled:opacity-0 disabled:pointer-events-none" title={t('edit') || 'Edit'}>
                                        ...
                                        <button onClick={() => onGenerateName(platform.id)} disabled={isBusy} className="p-0.5 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded text-indigo-400 hover:text-indigo-600 transition-colors cursor-pointer disabled:opacity-0 disabled:pointer-events-none" title="AI Generate Name">
                                    ...
                                    {!platform.is_active && (
                                        <span className="px-1 py-px text-[8px] uppercase font-bold tracking-widest text-slate-400 bg-slate-100 dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                                            {t('deactivated')}
                                        </span>
                                    )}
                                    {isBusy && (
                                        <span className="flex items-center gap-0.5 px-1 py-px text-[8px] uppercase font-bold tracking-widest text-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 rounded border border-indigo-200 dark:border-indigo-800/50">
                            ...
                            {isEditingUrl ? (
                                <div className="flex items-center gap-1 mt-0.5">
                                    ...
                                    />
                                    <button onClick={handleUrlSubmit} className="p-0.5 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded cursor-pointer">
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                                    </button>
                                    <button onClick={() => { setEditedUrl(platform.url); setIsEditingUrl(false); }} className="p-0.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded cursor-pointer">
                                ...
                                <div className="flex items-center gap-1 mt-0.5 group/url">
                                    <a href={platform.url} target="_blank" rel="noopener noreferrer"
                                        className="text-[10px] text-slate-400 hover:text-indigo-500 hover:underline truncate block transition-colors max-w-[calc(100%-16px)]"
                                        title={platform.url}
                                    >
                                        {platform.url}
                                    </a>
                                    <button onClick={() => !isBusy && setIsEditingUrl(true)} disabled={isBusy} className="opacity-0 group-hover/url:opacity-100 p-0.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded text-slate-400 hover:text-indigo-500 transition-all cursor-pointer disabled:hidden" title={t('edit') || 'Edit'}>
                                        <svg className="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                    </button>
                                </div>
                            )}
                ...
                {/* Right: action buttons + toggle (Centered relative to full card height) */}
                <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 self-stretch pl-2 sm:pl-3 border-l border-slate-100 dark:border-slate-800/60 transition-all duration-300 ml-1">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            {activeJob && (
                                <button 
                                    ...
                                    title={t('cancelCrawl')}
                                    className="w-8 h-8 flex items-center justify-center text-rose-500 bg-rose-50/50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-900/30 hover:bg-rose-100 dark:hover:bg-rose-500/20 rounded-lg transition-all duration-200 shadow-sm hover:shadow-rose-500/10 cursor-pointer"
                                >
                            ...
                            <button
                                ...
                                className="w-8 h-8 flex items-center justify-center text-slate-300 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg border border-transparent hover:border-rose-200 dark:hover:border-rose-800/50 transition-all duration-200 cursor-pointer disabled:opacity-30 disabled:pointer-events-none"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
