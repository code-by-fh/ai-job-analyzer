        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
          <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800 shrink-0">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                Pushover Template — {platform.name}
              </h3>
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer ml-3"
            >
              <svg
                className="w-4 h-4"
                fill="none"
          ...
          <div className="p-5 flex-1 overflow-auto">
            ...
            <p className="mt-3 text-[11px] text-slate-400 leading-relaxed">
              Standard-Nachricht wenn kein Template gesetzt:{" "}
              <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded font-mono">
                $company - Score: $match_score%\n\n$reasoning (gekürzt)...
              </code>
            </p>
          </div>
          ...
          <div className="flex justify-between items-center p-5 border-t border-slate-100 dark:border-slate-800 shrink-0">
            <div className="flex flex-col gap-1">
              ...
              {testStatus === "error" && testError && isAdmin && (
                <p className="text-[10px] text-rose-500 font-mono max-w-xs break-all">
                  {testError}
                </p>
              )}
            </div>
            <div className="flex gap-2">
              ...
              <button
                onClick={onSave}
                className="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-colors cursor-pointer shadow-sm"
              >
                Template speichern
              </button>
            </div>
