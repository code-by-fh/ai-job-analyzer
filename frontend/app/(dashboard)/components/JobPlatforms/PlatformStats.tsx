...
export default function PlatformStats({
  ...
  const [localDays, setLocalDays] = useState<number[]>(
    platform.schedule_days ?? [0, 1, 2, 3, 4],
  );
  ...
  const handleSave = () => {
    ...
    onScheduleChange(
      platform.id,
      hasSchedule ? localTime : null,
      hasSchedule ? localDays : null,
    );
    setOpen(false);
  };
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
      ...
      <button
        ...
        onClick={() =>
          router.push(
            `/listings?platform_id=${platform.id}&platform_name=${encodeURIComponent(platform.name)}`,
          )
        }
        title={t("jobsFound")}
      ...
      >
        <svg
          className="w-3 h-3 text-indigo-400 group-hover/chip:text-indigo-500 transition-colors shrink-0"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          ...
          >
            ...
            {active.length > 0 ? (
              <span className="font-medium">
                {active.map((a) => ADAPTER_SHORT[a] ?? a).join(" · ")}
              </span>
            ) : (
        ...
        >
          {lastRun.status === "failed" ? (
            ...
            <>
              ...
              <span>
                {lastRun.total_found ?? lastRun.total} {t("found")}
              </span>
          ...
          <svg
            className={`w-2.5 h-2.5 opacity-50 transition-transform duration-200 shrink-0 ${expandedLog === platform.url ? "rotate-180" : ""}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
