...
function isPlatformReady(platform: Platform, adapter: Adapter): boolean {
    switch (adapter) {
        case 'PUSHOVER': return true;
        case 'RESEND': return (platform.resend_recipients?.length ?? 0) > 0;
        case 'MAILJET': return (platform.mailjet_recipients?.length ?? 0) > 0;
        case 'SMTP': return (platform.smtp_recipients?.length ?? 0) > 0;
    }
}

function AdapterIcon({ adapter, active }: { adapter: Adapter; active: boolean }) {
...
export default function NotificationAdaptersModal({
    ...
    function openConfigModal(adapter: Adapter) {
        onClose();
        switch (adapter) {
            case 'PUSHOVER': onOpenPushoverModal(platform); break;
            case 'RESEND': onOpenResendModal(platform); break;
            case 'MAILJET': onOpenMailjetModal(platform); break;
            case 'SMTP': onOpenSmtpModal(platform); break;
        }
    }
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-sm overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800">
                        <div>
                            <h2 className="font-semibold text-slate-900 dark:text-white text-sm">Benachrichtigungsadapter</h2>
                            <p className="text-[11px] text-slate-400 mt-0.5 truncate max-w-[220px]">{platform.name}</p>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="p-4 space-y-2">
                        {ALL_ADAPTERS.map((adapter) => {
                            ...
                            const isActive = (platform.notification_adapters || []).includes(adapter);
                            const needsConfig = isSystemConfigured && isActive && !isPlatformReady(platform, adapter);
                            ...
                            return (
                                ...
                                >
                                    {/* Icon */}
                                    <div className={`w-8 h-8 flex items-center justify-center rounded-lg flex-shrink-0 transition-colors
                                        ${isActive
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className={`text-sm font-medium block ${isActive ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
