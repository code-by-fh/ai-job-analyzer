...
export default function EmailTemplateModal({
    ...
    const adapterTemplates = templates.filter(t => ['RESEND', 'MAILJET', 'SMTP'].includes(t.type));
    const [selectedTemplateId, setSelectedTemplateId] = useState<string>(() => {
        if (!templateValue) return '';
        const match = templates.find(t => ['RESEND', 'MAILJET', 'SMTP'].includes(t.type) && t.content === templateValue);
            ...
            >
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800 shrink-0">
                        <div>
                            <h3 className="font-bold text-slate-900 dark:text-white text-sm">{adapterName} E-Mail Template — {platform.name}</h3>
                            <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                                Loop: <code className="bg-slate-100 dark:bg-slate-800 px-1 rounded font-mono">{'{{#jobs}}'}</code>…<code className="bg-slate-100 dark:bg-slate-800 px-1 rounded font-mono">{'{{/jobs}}'}</code>
                                &nbsp;|&nbsp;
                                {['$title', '$company', '$match_score', '$reasoning', '$url', '$userName', '$jobCount'].map(v => (
                                    <code key={v} className="bg-slate-100 dark:bg-slate-800 px-1 rounded font-mono mr-1">{v}</code>
                                ))}
                            </p>
                        </div>
                        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer shrink-0 ml-3">
                    ...
                    <div className="p-5 flex-1 overflow-auto space-y-4 min-h-0">
                        ...
                        <div>
                            <p className="text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">
                                Empfänger (kommagetrennt) <span className="text-rose-500">*</span>
                            </p>
                            <input
                                type="text"
                            ...
                            />
                            {!recipientsValue.trim() && (
                                <p className="text-[10px] text-rose-500 mt-1">Pflichtfeld — ohne Empfänger kann keine Test-E-Mail gesendet werden.</p>
                            )}
                        </div>

                        {/* Template selector */}
                        <div>
                            <p className="text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Template laden</p>
                            {adapterTemplates.length > 0 ? (
                                ...
                                <p className="text-[11px] text-slate-400 italic">
                                    Keine Templates vorhanden — erstelle E-Mail-Templates unter <span className="font-semibold not-italic">Einstellungen → Benachrichtigungs-Templates</span>.
                                </p>
                            )}
                        </div>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-xs font-medium text-slate-700 dark:text-slate-300">HTML Template (optional)</p>
                                <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden text-[10px] font-bold">
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${tab === 'preview' ? 'bg-indigo-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                                    >
                                        Vorschau
                                    </button>
                                </div>
                            ...
                            {tab === 'editor' ? (
                                ...
                                <iframe
                                    ...
                                    style={{ height: '320px', resize: 'vertical', overflow: 'auto' }}
                            ...
                            {templateValue && tab === 'editor' && (
                                <button
                                    onClick={() => { onTemplateChange(''); setSelectedTemplateId(''); }}
                        ...
                        <p className="text-[11px] text-slate-400 leading-relaxed">
                            {tab === 'preview'
                                ? 'Vorschau mit Beispieldaten — echte Werte werden beim Versand eingesetzt.'
                    ...
                    <div className="flex justify-between items-center p-5 border-t border-slate-100 dark:border-slate-800 shrink-0">
                        <div className="flex flex-col gap-1">
                            ...
                            >
                                ...
                                {testStatus === 'ok' ? 'Gesendet!' : testStatus === 'error' ? 'Fehlgeschlagen' : 'Test-E-Mail senden'}
                            </button>
                            {testStatus === 'error' && testError && (
                                <p className="text-[10px] text-rose-500 max-w-xs break-all">{testError}</p>
                            )}
                        </div>
                        <div className="flex items-center gap-2">
                            {saveStatus === 'error' && saveError && (
                                <p className="text-[10px] text-rose-500 max-w-[180px] break-words">{saveError}</p>
                            )}
                            ...
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-bold rounded-lg transition-colors cursor-pointer shadow-sm disabled:opacity-60 flex items-center gap-1.5 ${
                                    saveStatus === 'saved' ? 'text-white bg-emerald-600' :
                                    saveStatus === 'error' ? 'text-white bg-rose-600 hover:bg-rose-500' :
                            ...
                            >
                                ...
                                {saveStatus === 'saved' && <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>}
