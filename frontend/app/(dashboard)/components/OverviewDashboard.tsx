...
export default function OverviewDashboard() {
  ...
  useEffect(() => {
    if (!token) return;
    Promise.all([
      fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/statistics`).then(
        (res) => {
          if (res.status === 401) {
            logout();
            return null;
      ...
      fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/settings-view`).then(
        (res) => {
      ...
      .then(([statsData, settingsData]) => {
        ...
        if (settingsData?.profile) {
          ...
          const adapters = [
            ...(p.pushover_user_key && p.pushover_api_token
              ? ["PUSHOVER"]
              : []),
            ...(p.resend_api_key && p.resend_from_email ? ["RESEND"] : []),
            ...(p.mailjet_api_key &&
            p.mailjet_secret_key &&
            p.mailjet_from_email
              ? ["MAILJET"]
              : []),
  ...
  const statCards = stats
    ? [
        ...
        },
        {
          ...
          color: "amber",
          icon: "🗓️",
          link: "/listings?status=INTERVIEW",
        },
        {
          label: t("statusOffer"),
          value: stats.offers,
      ...
      <PageWrapper>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="h-24 rounded-2xl bg-slate-100 dark:bg-slate-800/40 animate-pulse"
            />
          ))}
        </div>
  ...
  return (
    <PageWrapper>
      <PageHeader title={t("dashboard")} subtitle={t("dashboardDescription")} />
