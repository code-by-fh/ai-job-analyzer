...
export function useCrawl({
  ...
  const [activeCrawls, setActiveCrawls] =
    useState<Map<string, CrawlJob>>(initMap);
  ...
  const fetchCrawlStatus = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_URL}/scraper/crawl-status?user_id=${user.id}`,
      );
        ...
        try {
          ...
          if (data.type === "crawl_job_started") {
            ...
            if (data.user_id === user?.id) {
              setActiveCrawls((prev) => {
                ...
                return new Map(prev).set(data.job_id, {
                  ...
                  total: data.total,
                  total_found:
                    data.total_found ?? existing?.total_found ?? data.total,
            ...
            if (onNewJobRef.current)
              onNewJobRef.current(data.job, data.crawl_job_id);
            ...
            data.type === "interview_prep_ready" ||
            data.type === "company_profile_ready"
          ) {
            onJobEventRef.current?.({
              type: data.type,
              job_id: data.job_id,
              domain: data.domain,
            });
