...
import { Bell, Globe, Smartphone, Mail, Save, CheckCircle2, ShieldAlert, Clock, Info, ExternalLink, Send, X, HardDrive, Database, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    if (success === 'google_connected') {
      showSuccess(t('googleDriveConnected') || 'Google Drive connected successfully');
    }
  ...
  const handleGoogleDriveDisconnect = async () => {
    ...
    try {
      await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/storage/google/disconnect`, { method: 'POST' });
  ...
  const handleToggleStorage = async (service: 'NONE' | 'GOOGLE_DRIVE') => {
    try {
      const res = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/storage/toggle`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/60 p-1.5 rounded-2xl w-fit mb-8 border border-slate-200/60 dark:border-slate-800/60">
        {TABS.map(tab => (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl transition-all cursor-pointer text-sm font-bold ${activeTab === tab
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <h4 className="font-bold text-slate-900 dark:text-white mb-1">{t('internalStorage')}</h4>
                ...
                <div 
                  onClick={() => setStorageStatus(prev => ({ ...prev, service: 'GOOGLE_DRIVE' }))}
                  className={`relative p-5 rounded-2xl border-2 transition-all cursor-pointer group ${
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className={`p-2.5 rounded-xl ${
                      ...
                      <img src="https://upload.wikimedia.org/wikipedia/commons/1/12/Google_Drive_icon_%282020%29.svg" alt="G" className={`w-5 h-5 ${storageStatus.service === 'GOOGLE_DRIVE' ? 'brightness-0 invert' : ''}`} />
                    </div>
                    <div className="flex items-center gap-2">
                        {storageStatus.service === 'GOOGLE_DRIVE' && (
                          <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-indigo-500 text-white uppercase tracking-wider">
                        ...
                        {storageStatus.google_drive_email && storageStatus.service !== 'GOOGLE_DRIVE' && (
                  ...
                  <h4 className="font-bold text-slate-900 dark:text-white mb-1">{t('externalStorage')}</h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  ...
                  {showSetupGuide && (
                    <div className="mt-6 rounded-2xl border border-indigo-100 dark:border-indigo-900/40 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
                      <div className="px-5 py-3.5 bg-indigo-50/50 dark:bg-indigo-500/5 flex items-center justify-between border-b border-indigo-100 dark:border-indigo-900/40">
                        <span className="text-xs font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-widest">{t('storageSetupGuide')}</span>
                        <a href="https://console.cloud.google.com" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-bold">
                          Google Console <ExternalLink className="w-3.5 h-3.5" />
                        </a>
                      </div>
                        ...
                        <div className="space-y-6">
                          <div>
                            <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                               <span className="w-4 h-4 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[8px] text-slate-500">1</span>
                               {t('googleCloudProjectSetup')}
                            </h5>
                            <ol className="space-y-4">
                              {[t('googleStep1'), t('googleStep2'), t('googleStep3'), t('googleStep4'), t('googleStep5')].map((step, i) => (
                                <li key={i} className="flex items-start gap-3">
                                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-slate-50 dark:bg-slate-950/50 border border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-500 text-[10px] font-black flex items-center justify-center mt-0.5">{i+1}</span>
                                  <div className="space-y-2 flex-1">
                                    <p className="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed font-medium">{step}</p>
                                    {i === 4 && (
                                      <div className="p-3 bg-indigo-50/30 dark:bg-indigo-500/5 rounded-xl border border-indigo-100/50 dark:border-indigo-500/10 font-mono text-[11px] text-indigo-600 dark:text-indigo-400 break-all select-all flex justify-between items-center group/url">
                                        <span>{typeof window !== 'undefined' ? `${window.location.protocol}//${window.location.host.split(':')[0]}:8002/storage/google/callback` : 'http://localhost:8002/storage/google/callback'}</span>
                          ...
                          <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                            <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                               <span className="w-4 h-4 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[8px] text-slate-500">2</span>
                               {t('appConnection')}
                            </h5>
                            <ol className="space-y-3">
                              {[t('storageStep1'), t('storageStep2'), t('storageStep3')].map((step, i) => (
                                <li key={i} className="flex items-start gap-3">
                                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-[10px] font-black flex items-center justify-center mt-0.5 font-sans">{i + 1}</span>
                                  <span className="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed">{step}</span>
                                </li>
                              ))}
                            </ol>
                          </div>
