...
export default function UserManagementPage() {
  ...
  const executeDelete = async () => {
    ...
    try {
      await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_URL}/users/${userToDelete}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <PageWrapper>
      ...
      <PageHeader
        ...
        subtitle={t("userManagementDescription")}
      />
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {users.map((u) => (
              <tr
                key={u.id}
                className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition"
              >
                ...
                <td className="p-5">
                  {u.is_admin ? (
                    <span className="bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 px-2.5 py-1 rounded-lg text-xs font-bold border border-indigo-200 dark:border-indigo-500/30">
                      ADMIN
                    </span>
                  ) : (
                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 px-2.5 py-1 rounded-lg text-xs border border-slate-200 dark:border-slate-700">
                      USER
                    </span>
                  )}
                </td>
                <td className="p-5 text-right">
                  {!u.is_admin && (
                    <button
                      ...
                      className="text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 px-3 py-1.5 rounded-lg text-sm font-medium transition cursor-pointer"
                    >
                      {t("delete")}
                    </button>
                  )}
