...
function AccountCard({
  ...
  return (
    <section
      className={`relative z-10 bg-white dark:bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border ${isDanger ? "border-rose-200 dark:border-rose-900" : "border-slate-200 dark:border-slate-800"} ${className}`}
    >
      <div className="flex items-center gap-3 mb-6">
        {icon && (
          <div className={isDanger ? "text-rose-500" : "text-indigo-500"}>
            {icon}
          </div>
        )}
        <div>
          <h3
            className={`text-lg font-bold ${isDanger ? "text-rose-600 dark:text-rose-50" : "text-slate-800 dark:text-slate-100"}`}
          >
            {title}
          </h3>
          {subtitle && (
...
export default function Account() {
  ...
  const executeDeleteJobs = async () => {
    try {
      const res = await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_URL}/jobs?keep_favorites=${keepFavorites}&keep_applications=${keepApplications}`,
        {
          method: "DELETE",
        },
      );
  ...
  const executeFactoryReset = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${process.env.NEXT_PUBLIC_API_URL}/user/reset`,
        {
          method: "DELETE",
    ...
    <PageWrapper>
      <ConfirmModal
        ...
        onConfirm={() => {
          ...
          else if (confirmAction.type === "DELETE_PROFILE")
        ...
        confirmDisabled={
          confirmAction?.type === "FACTORY_RESET" && !resetPassword
        }
        ...
        {confirmAction?.type === "DELETE_JOBS" && (
          <div className="mt-4 space-y-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <label
                htmlFor="keepApplicationsCheckbox"
                className="text-sm text-slate-700 dark:text-slate-300 cursor-pointer font-medium"
              >
                {t("keepApplications")}
              </label>
      ...
      <PageHeader title={t("account")} subtitle={t("accountDescription")} />
